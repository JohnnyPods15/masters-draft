...
def show():
    ...
    st.markdown("---")
    tab1, tab2, tab3, tab4, tab5 = st.tabs(["👥 Team Names", "✏️ Edit Picks", "📊 Update Scores", "🔄 ESPN Sync", "⚙️ Draft Settings"])
    ...
    # ── TAB 4: ESPN SYNC ──
    with tab4:
        st.markdown("### 🔄 Sync Scores from ESPN")
        st.markdown("<div style='color:rgba(255,255,255,0.6);font-size:0.9rem;margin-bottom:1.5rem;'>Pulls live leaderboard data directly from ESPN during the tournament. Hit this once or twice a day to keep scores current.</div>", unsafe_allow_html=True)

        from espn_sync import fetch_espn_scores

        if st.button("⛳ Sync Live Scores from ESPN", use_container_width=True, type="primary"):
            with st.spinner("Fetching scores from ESPN..."):
                espn_scores, message = fetch_espn_scores()

            if espn_scores:
                existing_scores = load_scores()
                existing_scores.update(espn_scores)
                save_scores(existing_scores)
                st.success(f"✅ {message}")

                # Show preview of top 10
                st.markdown("**Top 10 on ESPN leaderboard:**")
                sorted_players = sorted(
                    [(name, data) for name, data in espn_scores.items() if data.get("position")],
                ...
                for name, data in sorted_players:
                    ...
                    thru = data.get("thru", "")
                    st.markdown(f"**{pos}.** {name} — {to_par} ({thru})")
            else:
                st.error(f"❌ {message}")
                st.info("You can still update scores manually in the **Update Scores** tab.")

    # ── TAB 5: DRAFT SETTINGS ──
    with tab5:
